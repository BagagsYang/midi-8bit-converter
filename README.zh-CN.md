...
生产 Vue UI 使用 `frontend/src/i18n/` 中的 JSON catalog 文件，覆盖英文、西班牙文、法文、日文和简体中文。旧 Flask 渲染 UI 使用 `legacy/web-flask/i18n/` 中的 catalog。修改生产前端 catalog 时，请保持 `en.json`、`es.json`、`fr.json`、`ja.json` 和 `zh-CN.json` 的键集合一致。英文是回退语言。仓库文档仍保持英文和简体中文两种语言。标准流程见 [docs/localisation.zh-CN.md](./docs/localisation.zh-CN.md)。
