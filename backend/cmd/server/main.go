...
func main() {
	...
	go func() {
		logger.Info("starting octabit go backend", "addr", server.Addr)
	...
	case signalValue := <-signalCh:
		logger.Info("stopping octabit go backend", "signal", signalValue.String())
	case err := <-errCh:
		if err != nil {
			logger.Error("octabit go backend failed", "error", err)
			os.Exit(1)
		}
		return
	}
	...
	if err := server.Shutdown(ctx); err != nil {
		logger.Error("octabit go backend shutdown failed", "error", err)
		os.Exit(1)
	}
	if err := <-errCh; err != nil {
		logger.Error("octabit go backend stopped with error", "error", err)
		os.Exit(1)
	}
}
