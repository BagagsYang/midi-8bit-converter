...
func TestLegacySynthesisJobFlow(t *testing.T) {
	...
	router := NewRouterWithOptions(config.Config{MaxUploadBytes: 1024 * 1024}, Options{LegacyJobs: legacyJobs})
	midiBytes, err := os.ReadFile(filepath.Join("..", "..", "testdata", "python-baseline", "midi", "simple.mid"))
	if err != nil {
		t.Fatal(err)
	}

	createRequest := legacySynthesisRequest(t, "lead.mid", midiBytes, map[string]string{
		"rate":        "44100",
		"layers_json": `[{"type":"sine","duty":0.5,"volume":1.0,"frequency_curve":[]}]`,
	})
	createResponse := httptest.NewRecorder()
	router.ServeHTTP(createResponse, createRequest)
	if createResponse.Code != http.StatusAccepted {
	...
	var createPayload jobs.Payload
	if err := json.Unmarshal(createResponse.Body.Bytes(), &createPayload); err != nil {
		t.Fatal(err)
	}
	if createPayload.Status != "ready" || createPayload.DownloadName != "lead_sine.wav" {
		t.Fatalf("create payload = %#v", createPayload)
	}
	if createPayload.DownloadURL != "/synthesise/jobs/"+createPayload.JobID+"/download" {
		t.Fatalf("download_url = %q", createPayload.DownloadURL)
	}
	if createPayload.DeleteURL != "/synthesise/jobs/"+createPayload.JobID {
		t.Fatalf("delete_url = %q", createPayload.DeleteURL)
	}

	pollResponse := httptest.NewRecorder()
	router.ServeHTTP(pollResponse, httptest.NewRequest(http.MethodGet, "/synthesise/jobs/"+createPayload.JobID, nil))
	if pollResponse.Code != http.StatusOK {
		t.Fatalf("poll status = %d body=%s", pollResponse.Code, pollResponse.Body.String())
	}

	downloadResponse := httptest.NewRecorder()
	router.ServeHTTP(downloadResponse, httptest.NewRequest(http.MethodGet, createPayload.DownloadURL, nil))
	if downloadResponse.Code != http.StatusOK {
		t.Fatalf("download status = %d body=%s", downloadResponse.Code, downloadResponse.Body.String())
	}
	if !bytes.HasPrefix(downloadResponse.Body.Bytes(), []byte("RIFF")) {
	...
	if disposition := downloadResponse.Header().Get("Content-Disposition"); disposition != "attachment; filename=lead_sine.wav" {
		t.Fatalf("Content-Disposition = %q", disposition)
	}

	deleteResponse := httptest.NewRecorder()
	router.ServeHTTP(deleteResponse, httptest.NewRequest(http.MethodDelete, createPayload.DeleteURL, nil))
	if deleteResponse.Code != http.StatusNoContent {
		t.Fatalf("delete status = %d body=%s", deleteResponse.Code, deleteResponse.Body.String())
	}
	expiredResponse := httptest.NewRecorder()
	router.ServeHTTP(expiredResponse, httptest.NewRequest(http.MethodGet, "/synthesise/jobs/"+createPayload.JobID, nil))
	if expiredResponse.Code != http.StatusGone {
		t.Fatalf("expired status = %d body=%s", expiredResponse.Code, expiredResponse.Body.String())
	}
	var expiredPayload map[string]string
	if err := json.Unmarshal(expiredResponse.Body.Bytes(), &expiredPayload); err != nil {
		t.Fatal(err)
	}
	if expiredPayload["status"] != "expired" {
		t.Fatalf("expired payload = %#v", expiredPayload)
	}
}

func TestLegacySynthesisJobValidationErrors(t *testing.T) {
	legacyJobs := jobs.NewLegacyService(t.TempDir(), jobs.Config{RunInline: true})
	router := NewRouterWithOptions(config.Config{MaxUploadBytes: 1024 * 1024}, Options{LegacyJobs: legacyJobs})
	...
	tests := []struct {
	...
	}{
		{
			...
			request: legacySynthesisRequest(t, "lead.txt", []byte("MThd"), map[string]string{"rate": "44100"}),
			status:  http.StatusBadRequest,
			message: "Unsupported file type. Upload a .mid or .midi file.",
		},
		{
			name:    "invalid rate",
			request: legacySynthesisRequest(t, "lead.mid", []byte("MThd"), map[string]string{"rate": "16000"}),
			status:  http.StatusBadRequest,
			message: "Unsupported sample rate. Choose 44100, 48000, or 96000.",
		},
		{
			name:    "empty midi",
			request: legacySynthesisRequest(t, "empty.mid", []byte{}, map[string]string{"rate": "44100"}),
			status:  http.StatusBadRequest,
			message: "Uploaded MIDI file is empty or incomplete.",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestSynchronousSynthesiseReturnsWAV(t *testing.T) {
	router := NewRouterWithOptions(config.Config{MaxUploadBytes: 1024 * 1024}, Options{})
	midiBytes, err := os.ReadFile(filepath.Join("..", "..", "testdata", "python-baseline", "midi", "simple.mid"))
	if err != nil {
		t.Fatal(err)
	}
	request := syncSynthesisRequest(t, "lead.mid", midiBytes, map[string]string{
		"rate":        "44100",
		"layers_json": `[{"type":"sine","duty":0.5,"volume":1.0,"frequency_curve":[]}]`,
	})
	response := httptest.NewRecorder()
	...
	if !bytes.HasPrefix(response.Body.Bytes(), []byte("RIFF")) {
		t.Fatalf("download prefix = %x", response.Body.Bytes()[:4])
	}
	if disposition := response.Header().Get("Content-Disposition"); disposition != "attachment; filename=lead_sine.wav" {
		t.Fatalf("Content-Disposition = %q", disposition)
	}
}

func TestSynchronousSynthesiseValidationErrors(t *testing.T) {
	router := NewRouterWithOptions(config.Config{MaxUploadBytes: 1024 * 1024}, Options{})
	tests := []struct {
		name    string
		request *http.Request
		...
		{
			name:    "unsupported extension",
			request: syncSynthesisRequest(t, "lead.txt", []byte("MThd"), map[string]string{"rate": "44100"}),
			status:  http.StatusBadRequest,
			message: "Unsupported file type. Upload a .mid or .midi file.",
		},
		{
			name:    "invalid rate",
			request: syncSynthesisRequest(t, "lead.mid", []byte("MThd"), map[string]string{"rate": "16000"}),
			status:  http.StatusBadRequest,
			message: "Unsupported sample rate. Choose 44100, 48000, or 96000.",
		},
		{
			name:    "empty midi",
			request: syncSynthesisRequest(t, "empty.mid", []byte{}, map[string]string{"rate": "44100"}),
			status:  http.StatusBadRequest,
			message: "Uploaded MIDI file is empty or incomplete.",
		},
	}
...
func legacySynthesisRequest(t *testing.T, name string, content []byte, fields map[string]string) *http.Request {
	return multipartRequest(t, "/synthesise/jobs", name, content, fields)
}

func syncSynthesisRequest(t *testing.T, name string, content []byte, fields map[string]string) *http.Request {
	return multipartRequest(t, "/synthesise", name, content, fields)
}
