...
func renderWAV(inputPath, outputPath, sourceName string, workspaceConfig Config) (string, int64, error) {
	profile, err := midi.ReadProfile(inputPath)
	if err != nil {
		return "", 0, err
	}
	runtimeLayers := RuntimeLayers(workspaceConfig)
	if requiresMultiChannel(runtimeLayers) && !profile.MultiChannel {
		return "", 0, fmt.Errorf("pro_requires_multi_channel_midi")
	}
	notes, err := midi.ReadNotesWithOptions(inputPath, midi.ReadOptions{IncludeDrumChannel: true})
	if err != nil {
		return "", 0, err
	}
