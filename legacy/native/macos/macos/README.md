...
3. Install the macOS build dependencies: `./.venv/bin/python3 -m pip install -r legacy/native/macos/requirements-build.txt`
4. Open `legacy/native/macos/MIDI8BitSynthesiser.xcodeproj` and run the `MIDI8BitSynthesiser` scheme.
...
- The Xcode build phase runs `legacy/native/macos/macos/build_desktop_resources.sh`.
- That script freezes `legacy/python-renderer/midi_to_wave.py` into a bundled helper binary with PyInstaller.
